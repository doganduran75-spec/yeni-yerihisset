...
import {
  Loader2, Search, Send, CheckCheck, Inbox,
  MessageSquare, Mail, Phone, ShoppingBag, BadgeCheck,
} from "lucide-react";
...
function MessagesInner() {
  ...
  useEffect(() => {
    if (selectedUserId) {
      router.replace(`/admin/messages?user=${selectedUserId}`, { scroll: false });
  ...
  async function fetchUsers() {
    ...
    const userStatsMap = new Map<string, { last_at: string; unread: number }>();
    ...
    const formatted: Profile[] = (profiles || []).map((p) => ({
      ...p,
  ...
  async function fetchUserDetail(userId: string) {
    ...
    const roles = (roleData || []).map((r: any) => r.roles).filter(Boolean);
    const orders = orderData || [];
    const orderTotal = orders.reduce((s: number, o: any) => s + (o.total_amount || 0), 0);
    setUserDetail({ roles, orderCount: orders.length, orderTotal });
  }
  ...
  async function handleSend() {
    ...
    try {
      const { data, error } = await (supabase as any)
        .from("messages")
        .insert({ user_id: selectedUserId, content: newMessage.trim(), sender_role: "admin" })
        .select()
        .single();
      ...
      if (selectedUser?.email) {
        await fetch("/api/crm/send-email", {
          ...
          body: JSON.stringify({ userId: selectedUserId, body: newMessage.trim() }),
        ...
        {selectedUserId ? (
          <>
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {userDetail?.roles.map((r) => (
                        <span
                          key={r.slug}
                          className="inline-flex items-center gap-1 text-[10px] font-bold bg-indigo-50 text-indigo-700 border border-indigo-200 px-2 py-0.5 rounded-full"
                        >
                          <BadgeCheck size={9} /> {r.name}
                        </span>
                      ))}
                    </div>
                    {/* İletişim + Sipariş özeti */}
                    <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1.5">
                      ...
                      {userDetail && (
                        <span className={cn(
                          "flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider border-l pl-3",
                          userDetail.orderCount > 0 ? "text-green-600" : "text-slate-400"
                        )}>
                          <ShoppingBag size={10} />
                          {userDetail.orderCount > 0
                            ? `${userDetail.orderCount} sipariş · ₺${userDetail.orderTotal.toFixed(2)}`
                            : "Henüz alışveriş yapmadı"}
                        </span>
                      )}
                    </div>
                  </div>
            ...
            <div className="p-4 border-t bg-white shrink-0">
              ...
              <p className="text-[10px] text-slate-400 mt-2 px-1 italic">
                Müşteri cevabınızı panelinde görecek ve e-posta ile bilgilendirilecektir.
              </p>
            </div>
          </>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 max-w-[250px] mt-1 mx-auto">
                Mesajlarını görüntülemek istediğiniz bir müşteri seçin.
              </p>
            </div>
          </div>
        )}
